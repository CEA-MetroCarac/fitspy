...
class FitSettings(QGroupBox):
    ...
    def initUI(self):
        ...
        self.fit_method_label = QLabel("Method:")
    ...
    def update_model(self, spectrum):
        # Find the key in FIT_METHODS that matches the value of
        # spectrum.fit_params["method"]
        method_value = spectrum.fit_params["method"]
        method_key = next((key for key, value in FIT_METHODS.items() if
                           value == method_value), None)
        if method_key is not None:
            self.fit_method_combo.setCurrentText(method_key)

        self.fit_negative_checkbox.setChecked(
            spectrum.fit_params["fit_negative"])
        self.fit_outliers_checkbox.setChecked(
            spectrum.fit_params["fit_outliers"])
        ...
        self.xtol_input.setValue(spectrum.fit_params["xtol"])
